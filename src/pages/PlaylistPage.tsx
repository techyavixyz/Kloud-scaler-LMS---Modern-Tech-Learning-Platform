...
const PlaylistPage = () => {
  ...
  const loadVideo = (index: number) => {
    if (!playlist[index] || !videoRef.current) return;

    const video = videoRef.current;
    const src = `http://localhost:3001${playlist[index].src}`;
            ...
            {playlist[currentIndex] && (
              <div className="mt-6 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-6">
                ...
                <p className="text-gray-300">
                  Learn the fundamentals and advanced concepts with hands-on demonstrations.
                </p>
              </div>
            )}
          </div>
